...
const xPrefScope = {};
Services.scriptloader.loadSubScript("chrome://userchromejs/content/xPref.jsm", xPrefScope);
const { xPref } = xPrefScope;
...
let _uc = {
  ...
  loadScript: function (script, win) {
    ...
    const matches = script.regex.test(href);
    const enabled = script.filename == this.ALWAYSEXECUTE || script.isEnabled;

    if (!matches || !enabled) {
      Services.console.logStringMessage(
        `UCJS skip ${script.filename} match=${matches} enabled=${enabled} href=${href}`
      );
      return;
    }
    ...
    if (script.onlyonce && script.isRunning) {
      ...
      Services.console.logStringMessage(`UCJS reuse onlyonce ${script.filename} href=${href}`);
      return;
    }

    try {
      Services.console.logStringMessage(`UCJS load ${script.filename} href=${href}`);
      Services.scriptloader.loadSubScript(script.url + '?' + script.file.lastModifiedTime,
                                          script.onlyonce ? { window: win } : win);
      script.isRunning = true;
      Services.console.logStringMessage(`UCJS loaded ${script.filename} href=${href}`);
      if (script.startup) {
        eval(script.startup);
      }
      if (!script.shutdown) {
      ...
      Cu.reportError(`UCJS failed to load ${script.filename}: ${ex}`);
    }
  },
...
let UserChrome_js = {
  windowObserver: function (subject, topic) {
    ...
    win.addEventListener('load', function onLoad() {
      ...
      try {
        Services.console.logStringMessage(`UCJS domwindowopened href=${win.location.href}`);
        UserChrome_js.load(win);
      } catch (ex) {
        Cu.reportError(`UCJS domwindowopened failed: ${ex}`);
      }
    }, false);
  },
...
if (!Services.appinfo.inSafeMode) {
  ...
  _uc.getScripts();
  Services.console.logStringMessage(`UCJS loader initialized with ${Object.keys(_uc.scripts).length} script(s)`);
  let windows = Services.wm.getEnumerator(null);
  while (windows.hasMoreElements()) {
    let win = windows.getNext();
    Services.console.logStringMessage(`UCJS startup enumerate href=${win.location.href}`);
    if (!('UC' in win))
      UserChrome_js.load(win)
  }
  Services.obs.addObserver(UserChrome_js, 'chrome-document-global-created', false);
  Services.obs.addObserver(UserChrome_js.windowObserver, 'domwindowopened', false);
}
